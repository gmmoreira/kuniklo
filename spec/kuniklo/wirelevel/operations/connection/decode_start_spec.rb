...
module Kuniklo
  RSpec.describe Wirelevel::Operations::Connection::DecodeStart do
    subject { Container['kuniklo.wirelevel.operations.connection.decode_start'].call(method.value) }

    describe 'from fixture' do
      let(:socket) { instance_double(Wirelevel::Socket) }
      let(:data) { read_fixture('connection_start.bin') }
      let(:frame) { Container['kuniklo.wirelevel.operations.read_frame'].call(socket) }
      let(:method) { Container['kuniklo.wirelevel.operations.decode_method'].call(frame.value) }
